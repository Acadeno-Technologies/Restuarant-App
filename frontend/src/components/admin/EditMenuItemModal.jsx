 ...
 * EditMenuItemModal
 *
 * Exact visual match and full feature parity with AddMenuItemModal:
 * - Header: "Edit Menu Dish" + subtitle + circular 'X' close button
 * - Dish Name (full width text input)
 * - Category Dropdown:
 *    - Custom styled dark dropdown with dynamic categories
 *    - "+ Add new category" option showing inline input + Add button
 * - Status Dropdown:
 *    - Custom styled dark dropdown with "Available" / "Unavailable" options
 * - Portion Pricing (₹) light peach card: Full Portion (*), Half Portion, Quarter
 * - Diet Type Dropdown:
 *    - Custom styled dark dropdown with "Veg", "Non-Veg", "Egg" + custom diet types
 *    - "+ Add new diet type" option showing inline input + Add button
 * - Spice Level Dropdown:
 *    - Custom styled dark dropdown with "Mild", "Medium", "Hot" + custom spice levels
 *    - "+ Add new spice level" option showing inline input + Add button
 * - Dish Photo (Upload from System / Computer) box with circular upload icon & preview
 * - Description multi-line textarea with matching placeholder
 * - Full-width dark maroon "Save Changes" button
 */
...
export const EditMenuItemModal = ({ isOpen, onClose, item, categories: initialCategories = [], onItemUpdated }) => {
  ...
  // Sync item data into form state
  useEffect(() => {
    if (isOpen && item) {
      setName(item.name || '');
      setCategoryId(
        item.category || (item.category_data && item.category_data.id) || (initialCategories[0]?.id || '')
      );
      setStatus(item.is_available !== false ? 'available' : 'unavailable');
      setPrice(item.price != null ? String(item.price) : '');
      setHalfPrice(item.half_price != null ? String(item.half_price) : '');
      setQuarterPrice(item.quarter_price != null ? String(item.quarter_price) : '');
      setDietType(item.diet_type ? toTitleCase(item.diet_type) : (item.is_veg ? 'Veg' : 'Non-Veg'));
      setSpiceLevel(item.spice_level ? toTitleCase(item.spice_level) : 'Medium');
    ...
    const loadData = async () => {
      try {
        ...
        if (Array.isArray(catData)) {
          ...
          if (item && (item.category || item.category_data?.id)) {
            setCategoryId(item.category || item.category_data.id);
          } else if (catData.length > 0 && !categoryId) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (imageFile) {
        ...
        payload.append('name', toTitleCase(name));
        if (categoryId) {
          payload.append('category', categoryId);
        }
        payload.append('price', parseFloat(price).toFixed(2));

        if (halfPrice && !isNaN(parseFloat(halfPrice)) && parseFloat(halfPrice) > 0) {
          payload.append('half_price', parseFloat(halfPrice).toFixed(2));
        } else {
          payload.append('half_price', '');
        }

        if (quarterPrice && !isNaN(parseFloat(quarterPrice)) && parseFloat(quarterPrice) > 0) {
          payload.append('quarter_price', parseFloat(quarterPrice).toFixed(2));
        } else {
          payload.append('quarter_price', '');
        }

        payload.append('is_veg', isVegBool);
        payload.append('diet_type', toTitleCase(dietType) || 'Veg');
        payload.append('spice_level', toTitleCase(spiceLevel) || 'Medium');
        payload.append('is_available', isAvailBool);
        ...
        payload = {
          ...
          half_price: halfPrice && !isNaN(parseFloat(halfPrice)) && parseFloat(halfPrice) > 0 ? parseFloat(halfPrice).toFixed(2) : null,
          quarter_price: quarterPrice && !isNaN(parseFloat(quarterPrice)) && parseFloat(quarterPrice) > 0 ? parseFloat(quarterPrice).toFixed(2) : null,
          is_veg: isVegBool,
          diet_type: toTitleCase(dietType) || 'Veg',
          spice_level: toTitleCase(spiceLevel) || 'Medium',
          is_available: isAvailBool,
      ...
      let detail = 'Failed to update dish. Please check your inputs and try again.';
    ...
    <div className="admin-add-dish-backdrop" onClick={onClose}>
      ...
      >
        ...
        <div className="admin-add-dish-header">
          <div>
            <h2 className="admin-add-dish-title">Edit Menu Dish</h2>
            <p className="admin-add-dish-subtitle">Update category, pricing, and dish details.</p>
          </div>
          <button
            type="button"
